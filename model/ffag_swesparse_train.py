#!/usr/bin/env python3
"""
FieldFormer-Autograd (sparse SWE): train from sensor observations.
...
class Config:
    ...
    grad_clip: float = 1.0
    patience: int = 12
    save: str = "/scratch/ab9738/fieldformer/model/ffag_swesparse_best.pt"
...
class FieldFormerSparseSWE(nn.Module):
    def __init__(self, d_model: int, nhead: int, layers: int, d_ff: int):
        ...
        self.input_proj = nn.Linear(4, d_model)  # dx,dy,dt + eta_neighbor
    ...
    def _forward_tokens(self, xyt_q: torch.Tensor, nb_xyt: torch.Tensor, nb_eta: torch.Tensor, Lx: float, Ly: float) -> torch.Tensor:
        ...
        with sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True):
            with torch.cuda.amp.autocast(enabled=False):
                h = self.encoder(self.input_proj(tokens))
        return self.head(h.mean(dim=1))

    def forward_observed(self, q_lin: torch.Tensor, obs_coords: torch.Tensor, obs_eta: torch.Tensor, nb_idx: torch.Tensor, Lx: float, Ly: float) -> torch.Tensor:
        return self._forward_tokens(obs_coords[q_lin], obs_coords[nb_idx], obs_eta[nb_idx], Lx=Lx, Ly=Ly)

    def forward_continuous(self, xyt_q: torch.Tensor, obs_coords: torch.Tensor, obs_eta: torch.Tensor, nb_idx: torch.Tensor, Lx: float, Ly: float) -> torch.Tensor:
        return self._forward_tokens(xyt_q, obs_coords[nb_idx], obs_eta[nb_idx], Lx=Lx, Ly=Ly)
