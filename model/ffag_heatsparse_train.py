#!/usr/bin/env python3
"""
FieldFormer-Autograd (sparse heat): train only from sensor observations.
...
Dataset contract (npz):
  - sensors_xy:  (S, 2)
  - sensor_noisy or sensor_clean: (S, Nt)
  - t: (Nt,)
  - params/param_names containing alpha_x, alpha_y (optional but expected)
  - x,y are optional (used only for domain bounds convenience)
"""
...
class Config:
    ...
    grad_clip: float = 1.0
    patience: int = 12
    save: str = "/scratch/ab9738/fieldformer/model/ffag_heatsparse_best.pt"
...
class SparseNeighborIndexer:
    ...
    Neighbors are generated from (all sensors) x (time offsets around k),
    capped to k_neighbors and excluding self when querying observed points.
    """

    def __init__(self, sensors_xy: torch.Tensor, t_grid: torch.Tensor, time_radius: int, k_neighbors: int):
        self.sensors_xy = sensors_xy  # (S,2)
        self.t_grid = t_grid          # (Nt,)
        ...
        self.base_sensor = s_mesh.reshape(-1)  # (S*(2R+1),)
    ...
    def gather_observed_neighbors(self, lin_q: torch.Tensor, exclude_self: bool = True) -> torch.Tensor:
        """Return neighbor lin indices, shape (B, K)."""
        s_q, k_q = self.lin_to_sk(lin_q)
        B = lin_q.shape[0]

        s_nb = self.base_sensor.to(lin_q.device).unsqueeze(0).expand(B, -1)
    ...
    def gather_continuous_neighbors(self, xyt_q: torch.Tensor) -> torch.Tensor:
        """
        For arbitrary query coords (B,3):
        - choose nearest time index k_hat from t_grid
        - use same sensor/time-offset pattern around k_hat
        """
        t_q = xyt_q[:, 2]
        ...
        B = xyt_q.shape[0]
        s_nb = self.base_sensor.to(xyt_q.device).unsqueeze(0).expand(B, -1)
...
class FieldFormerSparse(nn.Module):
    ...
    def _forward_tokens(self, xyt_q: torch.Tensor, nb_xyt: torch.Tensor, nb_vals: torch.Tensor,
                        Lx: float, Ly: float) -> torch.Tensor:
        dx = (nb_xyt[..., 0] - xyt_q[:, None, 0] + 0.5 * Lx) % Lx - 0.5 * Lx
        dy = (nb_xyt[..., 1] - xyt_q[:, None, 1] + 0.5 * Ly) % Ly - 0.5 * Ly
        dt = nb_xyt[..., 2] - xyt_q[:, None, 2]

        rel = torch.stack([dx, dy, dt], dim=-1)
        rel = rel * torch.exp(self.log_gammas)[None, None, :]
        tokens = torch.cat([rel, nb_vals[..., None]], dim=-1)

        # Force math SDPA path so higher-order autograd (uxx, uyy) is supported.
        # Efficient/flash kernels can fail on second derivatives with RuntimeError.
        with sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True):
            with torch.cuda.amp.autocast(enabled=False):
                h = self.input_proj(tokens)
                h = self.encoder(h)
        out = self.head(h.mean(dim=1)).squeeze(-1)
        return out

    def forward_observed(self, q_lin: torch.Tensor, obs_coords: torch.Tensor, obs_vals: torch.Tensor,
                         nb_idx: torch.Tensor, Lx: float, Ly: float) -> torch.Tensor:
        xyt_q = obs_coords[q_lin]
        nb_xyt = obs_coords[nb_idx]
        nb_vals = obs_vals[nb_idx]
        return self._forward_tokens(xyt_q, nb_xyt, nb_vals, Lx=Lx, Ly=Ly)

    def forward_continuous(self, xyt_q: torch.Tensor, obs_coords: torch.Tensor, obs_vals: torch.Tensor,
                           nb_idx: torch.Tensor, Lx: float, Ly: float) -> torch.Tensor:
        nb_xyt = obs_coords[nb_idx]
        nb_vals = obs_vals[nb_idx]
        return self._forward_tokens(xyt_q, nb_xyt, nb_vals, Lx=Lx, Ly=Ly)
...
def build_observed_tuples(sensors_xy: np.ndarray, t_grid: np.ndarray, sensor_values: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    S, Nt = sensor_values.shape
    x = np.repeat(sensors_xy[:, 0], Nt)
    y = np.repeat(sensors_xy[:, 1], Nt)
    t = np.tile(t_grid, S)
    coords = np.stack([x, y, t], axis=1).astype(np.float32)
    vals = sensor_values.reshape(-1).astype(np.float32)

    # deterministic sanity checks
    for s in [0, min(S - 1, 1), S - 1]:
        for k in [0, min(Nt - 1, 1), Nt - 1]:
            lin = s * Nt + k
...
def main(cfg: Config = CFG) -> None:
    ...
    S, Nt = sensor_values.shape
    assert t_np.shape[0] == Nt, "time grid length must match sensor series"

    # Optional consistency check with sensors_idx + x/y, if available
    ...
    # Domain extents for periodic deltas and collocation sampling
    ...
    def pde_residual_autograd(xyt: torch.Tensor) -> torch.Tensor:
        ...
        # forcing from generator: A*cos(pi x)*cos(pi y)*sin(4*pi*t/T), A=5, T=20
        A, T = 5.0, 20.0
        f = A * torch.cos(torch.pi * xyt[:, 0]) * torch.cos(torch.pi * xyt[:, 1]) * torch.sin(4 * torch.pi * xyt[:, 2] / T)
