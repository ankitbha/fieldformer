...
% \subsection{Baseline Selection and Scope}
% We evaluate FieldFormer against baselines that solve the same problem setting: continuous, mesh-free reconstruction of spatio-temporal fields from sparse and irregular sensor observations. In this setting, models operate directly on continuous coordinates $(x,y,t)$, support arbitrary query locations at inference time, and do not assume a fixed spatial discretization or sensor graph.

% This distinction is important. Many recent spatio-temporal models, such as ImputeFormer~\citep{nie2024imputeformer}, Senseiver~\citep{santos2023development}, RecFNO~\citep{zhao2024recfno}, HSPGNN \cite{liang2024higher}, FNO\citep{li2020fourier}, UNO\citep{rahman2022u}, Graph WaveNet\citep{wu2019graph}, DCRNN\citep{li2017diffusion}, and GNN-PINN~\citep{liu2023physics} variants, are mesh-dependent: they assume a fixed grid, rasterized domain, or static sensor graph, and treat missing values as masked entries rather than absent coordinates. As a result, they cannot natively (i) add new spatial locations at inference time, (ii) handle arbitrarily scattered sensors, or (iii) support resolution-independent continuous queries without additional rasterization or graph construction steps. Applying such methods in our setting would require resolution-specific design choices that fall outside the scope of our contributions and would lead to apples-to-oranges comparisons. For this reason, we restrict attention to mesh-free baselines that are directly applicable to sparse sensing field reconstruction.

% Although some of these mesh-free baselines were introduced several years ago, they remain the standard comparators for continuous coordinate-based field modeling and are still widely used in recent scientific machine learning and PDE reconstruction work~\cite{lee2025physics,chen2023implicit}. Their continued relevance stems from their resolution independence, architectural generality, and direct applicability to irregular sensor layouts.

% \noindent\textbf{Gaussian Processes:} Gaussian Processes (also referred to as Kriging) \citep{cressie1990origins} with separable spatio-temporal kernels represent the classical gold standard for interpolation from sparse measurements. They support arbitrary spatio-temporal queries and provide a principled probabilistic baseline grounded in geostatistics. Their primary limitation is computational scalability. To address this, we employ stochastic variational Gaussian processes (SVGPs) \citep{hensman2013gaussian}, which use inducing points to scale to large longitudinal datasets while retaining predictive flexibility. We tune the number of inducing points and kernel hyperparameters to ensure a competitive and well-optimized GP baseline.

% \noindent\textbf{Neural Fields:} Coordinate-based neural networks define a continuous surrogate function 
% $f(x,y,t)$ that can be evaluated at any query location. We consider two widely adopted neural field architectures that capture complementary modeling biases: Fourier-MLPs \citep{tancik2020fourier}, which use high-dimensional sinusoidal positional encodings to represent fine-scale and high-frequency structure, and SIRENs \citep{sitzmann2020implicit}, which employ sinusoidal activations throughout the network to directly model oscillatory and sharp spatio-temporal signals. Both models are trained using the initialization and optimization strategies recommended in their original works to ensure stable convergence. We additionally include physics-regularized training via PINN-style residual losses \cite{raissi2019physics} for these baselines where applicable, yielding strong physics-aware neural field comparators.

% Together, these baselines span the principal paradigms for mesh-free spatio-temporal field reconstruction: probabilistic interpolation (SVGP), coordinate-based neural representations (Fourier-MLP, SIREN), and physics-informed learning (PINN-style regularization). All baselines are implemented following best practices from the literature, with architectures and hyperparameters carefully tuned to their intended operating regimes.

% \noindent\textbf{Metrics:} We report root mean squared error (RMSE) and mean absolute error (MAE) on held-out sensor test sets as well as on full-field reconstructions where ground truth is available. These metrics directly measure reconstruction accuracy and are the primary indicators of knowledge recovery from sparse observations. Additional analysis of physical consistency, including relative PDE residual metrics, is provided in the appendix as a secondary diagnostic. While physics residuals can offer useful sanity checks, they are not treated as primary evaluation criteria, as overly smooth or biased solutions can achieve low residuals without accurately reconstructing the underlying field.
...
\multirow{7}{*}{Atmospheric: $U_x$/$V_y$}
...
\multirow{7}{*}{Pollution: PM$_{10}$/PM$_{2.5}$}
...
For real-world datasets, dense full-field ground truth is unavailable. We therefore use sensor-holdout evaluation as a proxy for spatial generalization: entire sensors are removed from training and used only for testing. Table~\ref{tab:global_proxy_real} reports these results for atmospheric and pollution monitoring datasets.

The real-world sensor-holdout results further demonstrate the prior-dependent nature of spatial extrapolation. On atmospheric variables, ImputeFormer and Senseiver perform better on temperature and relative humidity, while performance on wind components is mixed across methods. On the pollution monitoring dataset, ImputeFormer achieves the best sensor-holdout performance for PM$_{10}$ and PM$_{2.5}$, with Senseiver also performing strongly. FieldFormer is less dominant in this setting than in sensor-space imputation, consistent with the fact that sensor-holdout evaluation removes the local observational support that locality-aware methods rely on. These results do not contradict FieldFormer's sensor-space strengths; rather, they highlight that unseen-sensor reconstruction is a more underconstrained task where global latent priors can be beneficial when they match the structure of the dataset.

\begin{table}[htbp]
\centering
\caption{Sensor-holdout results on real-world sparse sensor benchmarks, used as a proxy for spatial generalization when dense full-field ground truth is unavailable. Results are reported as mean $\pm$ std over bootstrap samples. Lower is better. Note that Imputeformer is a fixed-node imputer, which gives it a transductive fixed-topology advantage in this setting.}
...
& SVGP & 2.033 $\pm$ 0.7474 & 0.9731 $\pm$ 0.2393 & \textbf{0.9001 $\pm$ 0.1931} & \textbf{0.5331 $\pm$ 0.07598} \\
...
Taken together, the global reconstruction and sensor-holdout results show that extrapolating away from observed sensor support is substantially more sensitive to the choice of inductive prior than imputing over an existing sensor network. FieldFormer performs strongly when local sensor support is available, while globally regularized baselines can be advantageous in some unseen-sensor settings. This supports our broader framing: under extreme sparse sensing, global reconstruction should be treated as prior-guided surrogate completion, whereas sensor-space imputation is the more identifiable and practically reliable objective.
...
For real-world datasets, we compare FieldFormer against FieldFormer-Gamma Field, since the PINN variants can be evaluated only on synthetic benchmarks, and the synthetic benchmarks already establish the importance of attention architecture. Table~\ref{tab:ablation_real_sparse} shows sensor-space imputation results on atmospheric and pollution monitoring datasets. Gamma Field improves several wind and pollution metrics, but does not uniformly improve all atmospheric variables. In particular, FieldFormer remains better for relative humidity and some mean absolute error metrics. This suggests that coordinate-dependent neighborhood geometry can help when local scales vary spatially, but also introduces additional flexibility that may not always be beneficial under sparse real-world sensing.
...
Overall, the ablations show that FieldFormer’s core locality-aware architecture is already strong for sensor-space imputation. Physics regularization does not consistently improve sparse prediction and can degrade performance when the residual objective conflicts with noisy or underconstrained observations. Replacing the transformer with an MLP preserves some benefits of adaptive neighborhood construction, but is less reliable on coupled or transport-dominated systems. Gamma Field improves selected global and heterogeneous real-world metrics, especially for some pollution and wind variables, but its gains are not uniform, suggesting that coordinate-dependent geometry is useful but should be evaluated on a case-by-case basis.
