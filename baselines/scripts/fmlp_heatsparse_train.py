#!/usr/bin/env python3
"""
Fourier-MLP (sparse heat): train only from sensor observations.
...
class Config:
    ...
    grad_clip: float = 1.0
    patience: int = 12
    save: str = "/scratch/ab9738/fieldformer/baselines/checkpoints/fmlp_heatsparse_best.pt"
...
def build_harmonics(k_max: int, device: torch.device) -> torch.Tensor:
    return torch.arange(1, k_max + 1, dtype=torch.float32, device=device)


class FourierMLPSparse(nn.Module):
    ...
    @staticmethod
    def _fourier_encode_1d(x: torch.Tensor, ks: torch.Tensor, length: float) -> torch.Tensor:
        denom = length if length > 0 else 1.0
        z = (2.0 * math.pi) * (x[..., None] / denom) * ks[None, :]
        return torch.cat([torch.sin(z), torch.cos(z)], dim=-1)

    def _fourier_encode_3d(self, xyt: torch.Tensor, Lx: float, Ly: float, Tt: float) -> torch.Tensor:
        dev = xyt.device
        ks_x = build_harmonics(self.kx, dev)
        ks_y = build_harmonics(self.ky, dev)
        ks_t = build_harmonics(self.kt, dev)
        fx = self._fourier_encode_1d(xyt[:, 0], ks_x, Lx)
        fy = self._fourier_encode_1d(xyt[:, 1], ks_y, Ly)
        ft = self._fourier_encode_1d(xyt[:, 2], ks_t, Tt)
        return torch.cat([fx, fy, ft], dim=-1)

    def _forward_xyt(self, xyt: torch.Tensor, Lx: float, Ly: float, Tt: float) -> torch.Tensor:
        feat = self._fourier_encode_3d(xyt, Lx=Lx, Ly=Ly, Tt=Tt)
    ...
    def forward_observed(self, q_lin: torch.Tensor, obs_coords: torch.Tensor, obs_vals: torch.Tensor, nb_idx: torch.Tensor, Lx: float, Ly: float, Tt: float) -> torch.Tensor:
        del obs_vals, nb_idx
        return self._forward_xyt(obs_coords[q_lin], Lx=Lx, Ly=Ly, Tt=Tt)
    ...
    def forward_continuous(self, xyt_q: torch.Tensor, obs_coords: torch.Tensor, obs_vals: torch.Tensor, nb_idx: torch.Tensor, Lx: float, Ly: float, Tt: float) -> torch.Tensor:
        ...
        return self._forward_xyt(xyt_q, Lx=Lx, Ly=Ly, Tt=Tt)
...
def main(cfg: Config = CFG) -> None:
    ...
    model = FourierMLPSparse(width=cfg.width, depth=cfg.depth, kx=cfg.kx, ky=cfg.ky, kt=cfg.kt).to(device)
    ...
    def predict_observed(q_lin: torch.Tensor) -> torch.Tensor:
        ...
        return model.forward_observed(q_lin, obs_coords, obs_vals, nb_idx, Lx=Lx, Ly=Ly, Tt=max(1e-6, t_max - t_min))
    ...
    def pde_residual_autograd(xyt: torch.Tensor) -> torch.Tensor:
        ...
        u = model.forward_continuous(xyt, obs_coords, obs_vals, nb_idx, Lx=Lx, Ly=Ly, Tt=max(1e-6, t_max - t_min))
    ...
    def periodic_bc_loss(n_bc: int, match_grad: bool) -> torch.Tensor:
        ...
        u0 = model.forward_continuous(xyt0, obs_coords, obs_vals, nb0, Lx=Lx, Ly=Ly, Tt=max(1e-6, t_max - t_min))
        uL = model.forward_continuous(xytL, obs_coords, obs_vals, nbL, Lx=Lx, Ly=Ly, Tt=max(1e-6, t_max - t_min))
        ...
        uy0 = model.forward_continuous(xyt0y, obs_coords, obs_vals, nb0y, Lx=Lx, Ly=Ly, Tt=max(1e-6, t_max - t_min))
        uyL = model.forward_continuous(xytLy, obs_coords, obs_vals, nbLy, Lx=Lx, Ly=Ly, Tt=max(1e-6, t_max - t_min))
