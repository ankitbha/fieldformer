#!/usr/bin/env python3
"""
Fourier-MLP (sparse pollution): train from sensor observations.
...
class FourierMLPSparsePollution(nn.Module):
    def __init__(self, width: int, depth: int, kx: int, ky: int, kt: int):
        super().__init__()
        self.kx = int(kx)
        self.ky = int(ky)
        self.kt = int(kt)
    ...
    @staticmethod
    def _encode_1d(z: torch.Tensor, k_max: int, length: float) -> torch.Tensor:
        ks = torch.arange(1, k_max + 1, dtype=torch.float32, device=z.device)
        angles = (2.0 * math.pi) * (z[..., None] / max(length, 1e-6)) * ks[None, :]
        return torch.cat([torch.sin(angles), torch.cos(angles)], dim=-1)

    def forward(self, xyt: torch.Tensor, x_min: float, y_min: float, t_min: float, Lx: float, Ly: float, Tt: float) -> torch.Tensor:
        x = xyt[:, 0] - x_min
        y = xyt[:, 1] - y_min
        t = xyt[:, 2] - t_min
        feat = torch.cat(
            [
                self._encode_1d(x, self.kx, Lx),
                self._encode_1d(y, self.ky, Ly),
                self._encode_1d(t, self.kt, Tt),
            ],
            dim=-1,
        )
...
def main(cfg: Config = CFG) -> None:
    ...
    model = FourierMLPSparsePollution(cfg.width, cfg.depth, cfg.kx, cfg.ky, cfg.kt).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=0.5, patience=3, min_lr=1e-6)
    stopper = EarlyStopping(cfg.patience)

    def predict(xyt: torch.Tensor) -> torch.Tensor:
        return model(xyt, x_min=x_min, y_min=y_min, t_min=t_min, Lx=Lx, Ly=Ly, Tt=Tt)
