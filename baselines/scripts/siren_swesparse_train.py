#!/usr/bin/env python3
"""
SIREN (sparse SWE): train from sensor observations.
...
class Config:
    ...
    grad_clip: float = 1.0
    patience: int = 12
    save: str = "/scratch/ab9738/fieldformer/baselines/checkpoints/siren_swesparse_nophysics_best.pt"
...
class SineLayer(nn.Linear):
    def __init__(self,in_features,out_features,w0=1.0,is_first=False):
        super().__init__(in_features,out_features); self.w0=w0; self.is_first=is_first
        with torch.no_grad():
            b=(1.0/in_features) if is_first else (math.sqrt(6.0/in_features)/w0)
            self.weight.uniform_(-b,b); self.bias.zero_()
    def forward(self,x): return torch.sin(self.w0*F.linear(x,self.weight,self.bias))


class SIRENSparseSWE(nn.Module):
    def __init__(self,width: int=256, depth:int=6, w0:float=30.0, w0_hidden:float=1.0):
        super().__init__()
        hs=[SineLayer(3,width,w0=w0,is_first=True)]
        for _ in range(depth-2): hs.append(SineLayer(width,width,w0=w0_hidden,is_first=False))
        self.hidden=nn.ModuleList(hs); self.final=nn.Linear(width,3)

    def _forward_xyt(self,xyt):
        h=xyt
...
def main(cfg: Config = CFG) -> None:
    ...
    model = SIRENSparseSWE().to(device)
