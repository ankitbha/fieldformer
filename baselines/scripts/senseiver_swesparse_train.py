#!/usr/bin/env python3
"""
Senseiver (sparse SWE): train only from sensor observations.

This is a standalone adaptation of the core Senseiver encoder/decoder idea for
the SWE sparse datasets used in this repository. It does not use the original
...
class Config:
    ...
    grad_clip: float = 1.0
    patience: int = 12
    save: str = "/scratch/ab9738/fieldformer/baselines/checkpoints/senseiver_swesparse_best.pt"
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="Train Senseiver on sparse SWE eta sensor observations.")
...
def main(cfg: Config) -> None:
    ...
    if cfg.obs_key in pack:
        ...
        sensor_values = pack["eta_sensor_noisy"].astype(np.float32)
    elif "eta_sensor_clean" in pack:
        sensor_values = pack["eta_sensor_clean"].astype(np.float32)
    else:
        raise KeyError("SWE sparse dataset must contain eta_sensor_noisy or eta_sensor_clean")
