...
filepath_locs_combined = f"{root}CombinedLocations.csv"
...
def load_combined_locations(path: str) -> pd.DataFrame:
    """Load the 60-sensor location file and expose Latitude/Longitude columns."""
    combined = pd.read_csv(path, index_col=0)
    lat_lon = combined["Lat-Lon"].str.split(",", n=1, expand=True).astype(float)
    combined["Latitude"] = lat_lon[0]
    combined["Longitude"] = lat_lon[1]
    return combined


# Load sensor data and build IC
locs = pd.read_csv(filepath_locs_gov, index_col=0)
sensor_locs_source = load_combined_locations(filepath_locs_combined)
...
# Map 60 sensor lon/lat locations -> nearest grid indices
sensor_indices = []
for xc, yc in zip(sensor_locs_source["Longitude"].to_numpy(), sensor_locs_source["Latitude"].to_numpy()):
...
# Collapse sensors that land in the same grid cell, preserving CSV order.
sensor_locs = list(dict.fromkeys(sensor_indices))
...
def generate_dataset(save_path: str = "pollution_dataset_60.npz") -> None:
