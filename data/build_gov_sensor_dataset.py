#!/usr/bin/env python3
"""Build a sparse real pollution sensor dataset from Delhi government CSVs."""
...
def _series_for_monitor(readings: pd.DataFrame, monitor_ids: list[str], timestamps: pd.DatetimeIndex) -> tuple[np.ndarray, np.ndarray]:
    subset = readings.loc[readings["monitor_id"].isin(monitor_ids), ["timestamp_round", *POLLUTANTS]]
    by_time = subset.groupby("timestamp_round", sort=True)[list(POLLUTANTS)].mean()
...
def build_dataset(data_csv: Path, locations_csv: Path, output_npz: Path) -> None:
    ...
    required_readings = {"monitor_id", "timestamp_round", *POLLUTANTS}
    ...
    values = np.empty((len(output_ids), len(timestamps), len(POLLUTANTS)), dtype=np.float32)
    ...
    np.savez_compressed(
        ...
        U_sensor=values,
        U_sensor_mask=mask,
        pollutant_names=np.asarray(POLLUTANTS, dtype="<U16"),
    ...
    print(f"       valid counts: {dict(zip(POLLUTANTS, valid_counts.tolist()))}")
    if merged_sensors:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=root / "gov_sensor_dataset.npz")
